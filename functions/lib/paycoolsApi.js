...
function getPaycoolsApiUrl(settings) {
...
function generateParamSignature(params, privateKey, algo = 'RSA-SHA256', format = 'json', explicitAppId) {
...
function verifySignature(params, signature, publicKey, algo = 'RSA-SHA256') {
    const signString = Object.keys(params)
        .filter(k => k !== 'sign' && k !== 'signType' && params[k] !== undefined && params[k] !== null && params[k] !== '')
        .sort()
        .map(k => `${k}=${params[k]}`)
...
async function verifyIsAdmin(req) {
    try {
        ...
        return (userData?.adminRole === 'admin' ||
...
async function addAuditLog(adminUserId, action, module, before, after, req) {
    try {
...
async function postPaycools(url, body) {
...
exports.paycoolsApi = (0, https_1.onRequest)({
    cors: true,
    ...
    try {
            ...
            if (isPaidTransition) {
                ...
                if (paymentType === 'accreditation') {
                    ...
                    if (uid && selectedMembershipType !== 'None') {
                        const membershipFees = { Regular: 5000, Associate: 3500, Institutional: 10000, Affiliate: 4500 };
                    ...
                    if (regSnap.exists) {
                        ...
                        await db.collection('member_notifications').add({
                            ...
                            read: false,
                            createdAt: admin.firestore.FieldValue.serverTimestamp()
                        }).catch((err) => console.error('[paycoolsApi Webhook] Member notification error:', err));
                        await db.collection('admin_notifications').add({
                            ...
                            read: false,
                            createdAt: admin.firestore.FieldValue.serverTimestamp()
                        }).catch((err) => console.error('[paycoolsApi Webhook] Admin notification error:', err));
        ...
        if (path === '/settings/test-connection' && req.method === 'POST') {
            ...
            const generateTestSign = (algo, format) => {
                const filteredParams = {};
                Object.keys(params)
                    .filter(k => k !== 'appId' && k !== 'sign' && k !== 'signType' && params[k] !== undefined && params[k] !== null && params[k] !== '')
                    .forEach(k => {
                ...
                if (format === 'json') {
                    ...
                    const queryParams = { ...filteredParams, appId: settings.appId };
            ...
            const attempts = [
                { name: 'RSA-SHA256_JSON', algo: 'RSA-SHA256', format: 'json' },
                { name: 'RSA-SHA1_JSON', algo: 'RSA-SHA1', format: 'json' },
                { name: 'RSA-SHA256_QUERY', algo: 'RSA-SHA256', format: 'query' },
                { name: 'RSA-SHA1_QUERY', algo: 'RSA-SHA1', format: 'query' }
            ];
        ...
        if (path === '/checkout/create' && req.method === 'POST') {
            const { orderId, amount, currency, description, successUrl, cancelUrl, callbackUrl, metadata, 
            // Accreditation-specific
            paymentType, accreditationAppId, selectedMembershipType } = req.body;
...
//# sourceMappingURL=paycoolsApi.js.map