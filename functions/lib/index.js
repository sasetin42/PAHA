...
exports.paycoolsApi = exports.deleteMemberAccount = exports.paycoolsValidate = exports.createPaycoolsQR = exports.createPaycoolsPayment = void 0;
...
// ─── RSA-SHA256 Signing ───────────────────────────────────────────────────
function generatePaycoolsSignature(params, privateKey) {
...
exports.paycoolsValidate = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    try {
        const body = req.body;
...
async function isCallerAdmin(uid, email) {
...
async function verifyAdmin(request) {
    ...
    let callerEmail = request.auth?.token?.email;
    if (!callerUid && request.data?.idToken) {
        try {
            ...
            throw new https_1.HttpsError('unauthenticated', 'Invalid or expired admin identity token.');
        }
    }
    if (!callerUid) {
        throw new https_1.HttpsError('unauthenticated', 'You must be signed in as an administrator.');
    }
    if (!(await isCallerAdmin(callerUid, callerEmail))) {
        throw new https_1.HttpsError('permission-denied', 'Only administrators may permanently delete accounts.');
    }
 ...
 */
exports.deleteMemberAccount = (0, https_1.onCall)(async (request) => {
    ...
    if (!uid && !email) {
        throw new https_1.HttpsError('invalid-argument', 'A uid or email is required.');
    }
    ...
    if (uid === adminUid) {
        throw new https_1.HttpsError('failed-precondition', 'You cannot delete your own admin account.');
    }
    const deletedCollections = {};
    const deleteByField = async (colName, field, value) => {
    ...
    const deleteByUidOrEmail = async (colName) => {
        let total = 0;
        if (uid) {
        ...
        if (email)
            total += await deleteByField(colName, 'email', email);
        return total;
    };
    ...
    if (uid)
        deletedCollections.accreditation_applications += await deleteByField('accreditation_applications', 'clinicId', uid);
    ...
    if (email)
        deletedCollections.event_registrations += await deleteByField('event_registrations', 'attendeeEmail', email);
    ...
    if (email)
        deletedCollections.members = await deleteByField('members', 'email', email);
    if (uid) {
    ...
    if (uid)
        deletedCollections.admin_notifications = await deleteByField('admin_notifications', 'uid', uid);
    // 8. committee_members
